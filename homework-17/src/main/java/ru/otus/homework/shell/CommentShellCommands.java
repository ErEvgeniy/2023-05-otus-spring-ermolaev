...
public class CommentShellCommands {
	...
	@ShellMethod(value = "Print comment list for book", key = {"clb", "comment-list-book"})
	public String commentListByBook(@ShellOption String bookId) {
		List<Comment> comments = commentService.findAllCommentsByBook(bookId);
		StringBuilder builder = new StringBuilder("Comments for book ")
			.append(bookId).append(":").append(System.lineSeparator());
		for (Comment comment : comments) {
			builder.append(commentConverter.getCommentWithId(comment)).append(System.lineSeparator());
		}
		return builder.toString();
	...
	public String commentById(@ShellOption String commentId) {
		try {
	...
	public String commentDelete(@ShellOption String commentId) {
		try {
			commentService.deleteCommentById(commentId);
			return String.format("Comment with id: %s successfully deleted", commentId);
		} catch (DataIntegrityViolationException ex) {
			return String.format("Comment with id: %s is used. Unable to delete", commentId);
		}
	}
	...
	public String commentCreate(
		...
		@ShellOption(value = { "-b", "--bookId" }) String bookId
	) {
